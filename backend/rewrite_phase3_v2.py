...
@router.get("/{module_name}", response_model=PaginatedResponse[{schema_read}])
...
{rls_code}
    ...
    if sort and hasattr({model_class}, sort):
        sort_attr = getattr({model_class}, sort)
        if order == "desc":
...
# Update imports to include PaginatedResponse
...
if start_idx != -1:
    ...
    with open('app/api/crm.py', 'w') as f:
        f.write(new_content)
    print("Rewritten successfully with Enterprise Pagination")
else:
