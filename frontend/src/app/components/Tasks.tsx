...
export function Tasks() {
  ...
  return (
    <div className="flex flex-col h-full bg-background text-foreground" style={{ fontFamily: "var(--font-sans)" }}>
      {/* Top Header Stats */}
      <div className="flex items-center gap-6 px-6 py-4 border-b border-border shrink-0 bg-card">
        <div className="flex items-center gap-2">
          <span className="text-xl font-medium text-foreground">{tasks.length}</span>
          <span className="text-sm font-medium text-muted-foreground">Total</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xl font-medium text-rose-500">{countOf("Overdue")}</span>
          <span className="text-sm font-medium text-rose-500">Overdue</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xl font-medium text-amber-500">{countOf("Today")}</span>
          <span className="text-sm font-medium text-amber-500">Due today</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xl font-medium text-primary">{tasks.filter(t => t.status === "in_progress").length}</span>
          <span className="text-sm font-medium text-primary">In progress</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xl font-medium text-teal-500">{countOf("Done")}</span>
          <span className="text-sm font-medium text-teal-500">Completed</span>
        </div>
        
        <div className="ml-auto">
          <button onClick={() => openModal()} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-primary text-primary-foreground hover:bg-primary/90 rounded transition-colors shadow-xs">
      ...
      {/* Toolbar */}
      <div className="flex items-center gap-4 px-6 py-3 border-b border-border shrink-0 bg-card/50">
        <div className="flex items-center gap-1">
          {tabs.map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-medium rounded transition-colors ${tab === t ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground hover:bg-secondary"}`}
            >
              {t}
              {(t === "Today" || t === "Overdue" || t === "Upcoming") && (
                <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded-full ${tab === t ? "bg-white/20 text-white" : "bg-secondary text-muted-foreground"}`}>
                  {countOf(t)}
                </span>
              )}
            </button>
          ...
          <table className="w-full text-[11px] border-collapse min-w-[1000px]">
            <thead className="sticky top-0 z-10 bg-card border-b border-border">
              <tr>
                ...
                <th className="w-24 px-3 py-3 text-left font-semibold text-muted-foreground uppercase tracking-wider">Priority</th>
                <th className="px-3 py-3 text-left font-semibold text-muted-foreground uppercase tracking-wider">Task</th>
                <th className="w-32 px-3 py-3 text-left font-semibold text-muted-foreground uppercase tracking-wider">Type</th>
                <th className="w-48 px-3 py-3 text-left font-semibold text-muted-foreground uppercase tracking-wider">Contact</th>
                <th className="w-32 px-3 py-3 text-left font-semibold text-muted-foreground uppercase tracking-wider">Status</th>
                <th className="w-36 px-3 py-3 text-left font-semibold text-muted-foreground uppercase tracking-wider">Due</th>
                <th className="w-32 px-3 py-3 text-left font-semibold text-muted-foreground uppercase tracking-wider">Assignee</th>
            ...
            <tbody>
              {filtered.map(task => {
                ...
                return (
                  <tr key={task.id} className={`border-b border-border transition-colors ${isDone ? "opacity-40" : "hover:bg-secondary/20"}`}>
                    <td className="px-4 py-3 align-top pt-4">
                      <button onClick={() => toggleCheck(task.id, task.status)} className="text-muted-foreground hover:text-emerald-500 transition-colors">
                    ...
                    <td className="px-3 py-3 align-top pt-3.5">
                      <p className={`font-medium text-foreground text-xs ${isDone ? "line-through" : ""}`}>{task.title}</p>
                      {task.description && (
                        <p className="text-[11px] text-muted-foreground mt-1 line-clamp-1 flex items-center gap-1">
                          <span className="px-1.5 border border-border rounded text-[9px] uppercase bg-secondary/50">{task.task_type || "task"}</span>
                          {task.description}
                        </p>
                      )}
                    </td>
                    ...
                    <td className="px-3 py-3 align-top pt-3.5">
                      {(contact || account) ? (
                        <div className="flex flex-col">
                          {contact && <span className="font-medium text-foreground">{contact.first_name} {contact.last_name}</span>}
                          {account && <span className="text-muted-foreground text-[10px]">{account.name}</span>}
                        </div>
                      ) : (
                    ...
                    <td className="px-3 py-3 align-top pt-4">
                      <span className={`flex items-center gap-1 font-mono text-[11px] ${isOverdue ? "text-rose-500" : "text-muted-foreground"}`}>
                    ...
                    <td className="px-3 py-3 align-top pt-3.5">
                      {user ? (
                        <div className="flex items-center gap-1.5">
                          <div className="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center text-[9px] font-mono font-semibold text-primary">
                    ...
                    <td className="px-3 py-3 align-top pt-3.5 text-right">
                      <button onClick={() => openModal(task)} className="p-1 text-muted-foreground hover:text-foreground transition-colors rounded hover:bg-secondary">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            <div className="px-5 py-4 border-b border-border flex items-center justify-between bg-muted/30">
              <h2 className="text-sm font-semibold text-foreground">{selectedTask ? "Edit Task" : "New Task"}</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-muted-foreground hover:text-foreground"><X size={16}/></button>
            </div>
            
            <div className="p-5 overflow-y-auto max-h-[70vh] flex flex-col gap-4 text-xs">
              <div className="flex flex-col gap-1.5">
                <label className="text-muted-foreground font-medium">Task Title</label>
                <input
                  type="text"
                  value={formData.title}
                  onChange={e => setFormData({...formData, title: e.target.value})}
                  className="px-3 py-2 bg-secondary/30 border border-border rounded text-foreground focus:border-primary focus:outline-none"
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  className="px-3 py-2 bg-secondary/30 border border-border rounded text-foreground focus:border-primary focus:outline-none h-20 resize-none"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-muted-foreground font-medium">Due Date &amp; Time</label>
                  <input
                    type="datetime-local"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-muted-foreground font-medium">Customer (Account)</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-muted-foreground font-medium">Contact Person</label>
                  <select
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-muted-foreground font-medium">Assign To</label>
                  <select
            ...
            <div className="px-5 py-4 border-t border-border flex justify-between bg-muted/30">
              {selectedTask ? (
                <button onClick={() => deleteTask(selectedTask.id)} className="px-4 py-2 text-xs font-medium text-rose-500 hover:bg-rose-500/10 rounded transition-colors">
                  Delete
                </button>
              ) : <div></div>}
              <div className="flex gap-2">
                <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors">
                  Cancel
                </button>
                <button onClick={saveTask} disabled={!formData.title} className="px-4 py-2 text-xs font-medium bg-primary text-primary-foreground rounded hover:bg-primary/90 transition-colors disabled:opacity-50">
                  Save Task
                </button>
              </div>
            </div>
